...
@Expected NVARCHAR (MAX), @Actual NVARCHAR (MAX), @ResultTable NVARCHAR (MAX), @ColumnList NVARCHAR (MAX), @MatchIndicatorColumnName NVARCHAR (MAX)
AS
BEGIN
    DECLARE @cmd AS NVARCHAR (MAX);
    DECLARE @RestoredRowIndexCounterColName AS NVARCHAR (MAX);
    ...
    SET @cmd = 'SET @r = 
         CASE WHEN EXISTS(
                  SELECT 1 
                    FROM ' + @ResultTable + ' WHERE ' + @MatchIndicatorColumnName + ' IN (''<'', ''>'')) 
              THEN 1 ELSE 0 
         END';
    DECLARE @UnequalRowsExist AS INT;
    EXECUTE sp_executesql @cmd, N'@r INT OUTPUT', @UnequalRowsExist OUTPUT;
