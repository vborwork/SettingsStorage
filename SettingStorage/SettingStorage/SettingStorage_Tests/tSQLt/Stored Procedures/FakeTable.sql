...
@TableName NVARCHAR (MAX), @SchemaName NVARCHAR (MAX)=NULL, @Identity BIT=NULL, @ComputedColumns BIT=NULL, @Defaults BIT=NULL
AS
BEGIN
    DECLARE @OrigSchemaName AS NVARCHAR (MAX);
    DECLARE @OrigTableName AS NVARCHAR (MAX);
    DECLARE @NewNameOfOriginalTable AS NVARCHAR (4000);
    DECLARE @OrigTableFullName AS NVARCHAR (MAX);
    ...
    EXECUTE tSQLt.Private_ValidateFakeTableParameters @SchemaName, @OrigTableName, @OrigSchemaName;
    EXECUTE tSQLt.Private_RenameObjectToUniqueName @SchemaName, @TableName, @NewNameOfOriginalTable OUTPUT;
    ...
    ELSE
        ...
        END
    EXECUTE tSQLt.Private_CreateFakeOfTable @SchemaName, @TableName, @OrigTableFullName, @Identity, @ComputedColumns, @Defaults;
    EXECUTE tSQLt.Private_MarkFakeTable @SchemaName, @TableName, @NewNameOfOriginalTable;
END
